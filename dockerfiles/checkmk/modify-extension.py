...
# Include "os" to check for correct copy of the kubernetes_namespaces agent before changing lines in it below
import os
...
if len(argv) > 2:
    ...
    # New code to update version number inside the kubernetes_namespaces script itself
    kubernetes_namespaces_agent_path = "/omd/sites/cmk/local/share/check_mk/agents/plugins/kubernetes_namespaces"
    if os.path.exists(kubernetes_namespaces_agent_path):
        with open(kubernetes_namespaces_agent_path, "r+") as file:
            content = file.read().replace('CMK_VERSION="0.0.0"', f'CMK_VERSION="{version}"')
            file.seek(0)
            file.write(content)
            file.truncate()
    else:
        print(f"File not found: {kubernetes_namespaces_agent_path}")
    # End of new code
